...
> Spec 列标注说明：`✅ treespec` = 已有 treespec 覆盖，`📝 vitest` = adapter/SSE/protocol 层由 vitest 覆盖，`📋 spec` = 仅 spec.md（tc 已删，待补 YAML）。
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 2.1 | 创建 session（happy path） | `POST /sessions` | `sumeru session add <proto> [--project <p>] [--task <t>]` | ✅ treespec |
| 2.2 | 创建 session（minimal, no project/task） | `POST /sessions` (`project`/`task` omitted) | `sumeru session add <proto>` | ✅ treespec |
...
| 2.5 | 列出所有 sessions | `GET /sessions` | `sumeru session list` | ✅ treespec |
| 2.6 | 获取 session 详情 | `GET /sessions/:id` | `sumeru session get <id>` | ✅ treespec |
| 2.7 | 停止 running session | `POST /sessions/:id/stop` | `sumeru session stop <id>` | ✅ treespec |
| 2.8 | 停止已 idle session（409） | `POST /sessions/:id/stop` → 409 | `sumeru session stop <id>` → error | ✅ treespec + `errors/error-paths.test.yaml` |
| 2.9 | 删除 idle session | `DELETE /sessions/:id` | `sumeru session remove <id>` | ✅ treespec |
| 2.10 | 删除 running session | `DELETE /sessions/:id` | `sumeru session remove <id>` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 3.1 | 向 idle session 发消息恢复执行 | `POST /sessions/:id/messages` | `sumeru session send <id> "msg"` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 5.1 | 全量查询 turns | `GET /sessions/:id/turns` | `sumeru session turns <id>` | ✅ treespec |
| 5.2 | 分页查询（after=N） | `GET /sessions/:id/turns?after=N` | `sumeru session turns <id> --after N` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 6.1 | 列出 providers | `GET /providers` | `sumeru provider list` | ✅ treespec |
| 6.2 | 获取 provider 详情 | `GET /providers/:name` | `sumeru provider get <name>` | ✅ treespec |
| 6.3 | 创建 provider | `PUT /providers/:name` | `sumeru provider add <name> --api-type --base-url` | ✅ treespec |
| 6.4 | 更新 provider | `PUT /providers/:name` | `sumeru provider update <name> --api-type/--base-url/--api-key` | ✅ treespec |
| 6.5 | 删除 provider | `DELETE /providers/:name` | `sumeru provider remove <name>` | ✅ treespec |
| 6.6 | 删除被 Model 引用的 provider（409） | `DELETE /providers/:name` → 409 | `sumeru provider remove <name>` → error | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 7.1 | 列出 models | `GET /models` | `sumeru model list [--provider <name>]` | ✅ treespec |
| 7.2 | 获取 model 详情 | `GET /models/:name` | `sumeru model get <name>` | ✅ treespec |
| 7.3 | 创建 model | `PUT /models/:name` | `sumeru model add <name> --provider <name> --model <api-model>` | ✅ treespec |
| 7.4 | 更新 model | `PUT /models/:name` | `sumeru model update <name> --provider/--model/--context-window` | ✅ treespec |
| 7.5 | 删除 model | `DELETE /models/:name` | `sumeru model remove <name>` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 8.1 | 列出 personas | `GET /personas` | `sumeru persona list` | ✅ treespec |
| 8.2 | 获取 persona 详情 | `GET /personas/:name` | `sumeru persona get <name>` | ✅ treespec |
| 8.3 | 创建 persona | `PUT /personas/:name` | `sumeru persona add <name> --instructions` | ✅ treespec |
| 8.4 | 删除 persona | `DELETE /personas/:name` | `sumeru persona remove <name>` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 9.1 | 列出 prototypes | `GET /prototypes` | `sumeru prototype list` | ✅ treespec |
| 9.2 | 删除 prototype | `DELETE /prototypes/:name` | `sumeru prototype remove <name>` | ✅ treespec |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 10.1 | 列出 adapters | `GET /adapters` | `sumeru adapter list` | ✅ treespec |
| 10.2 | 获取 adapter 详情 | `GET /adapters/:name` | `sumeru adapter get <name>` | ✅ treespec |
...
| 11.2 | 容器内执行 shell | `POST /sessions/:id/commands` `{"type":"exec",...}` | `sumeru session exec <id> -- <command...>` | ✅ treespec |
| 11.3 | 切换 model | `POST /sessions/:id/commands` `{"type":"model",...}` | `sumeru session model <id> <model-id>` | ✅ treespec |
| 11.4 | 清上下文 | `POST /sessions/:id/commands` `{"type":"reset",...}` | `sumeru session reset <id> [--persona]` | ✅ treespec |
| 11.5 | snapshot（docker commit） | `POST /sessions/:id/commands` `{"type":"snapshot",...}` | `sumeru session snapshot <id> <name>` | ✅ treespec |
| 11.6 | snapshot 输出可读性 | — | `sumeru session snapshot` 多行格式 | ✅ treespec |
...
| 14.4 | Host 重启后 session 上下文恢复 | — (adapter JSONL 持久化) | `server restart` → `session send` 上下文不丢 | ✅ treespec |
