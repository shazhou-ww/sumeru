...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 2.1 | 创建 session（happy path） | `POST /sessions` | `sumeru session add <proto> [--project <p>] [--task <t>]` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
| 2.2 | 创建 session（minimal, no project/task） | `POST /sessions` (`project`/`task` omitted) | `sumeru session add <proto>` | ✅ atest: `session/lifecycle/session-create-no-task.yaml` |
...
| 2.5 | 列出所有 sessions | `GET /sessions` | `sumeru session list` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
| 2.6 | 获取 session 详情 | `GET /sessions/:id` | `sumeru session get <id>` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
| 2.7 | 停止 running session | `POST /sessions/:id/stop` | `sumeru session stop <id>` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
| 2.8 | 停止已 idle session（409） | `POST /sessions/:id/stop` → 409 | `sumeru session stop <id>` → error | ✅ atest: `session/lifecycle/session-lifecycle.yaml` + `errors/error-paths.yaml` |
| 2.9 | 删除 idle session | `DELETE /sessions/:id` | `sumeru session remove <id>` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
| 2.10 | 删除 running session | `DELETE /sessions/:id` | `sumeru session remove <id>` | ✅ atest: `session/lifecycle/session-delete-running.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 3.1 | 向 idle session 发消息恢复执行 | `POST /sessions/:id/messages` | `sumeru session send <id> "msg"` | ✅ atest: `session/lifecycle/session-lifecycle.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 5.1 | 全量查询 turns | `GET /sessions/:id/turns` | `sumeru session turns <id>` | ✅ atest: `turns/turns-pagination.yaml` |
| 5.2 | 分页查询（after=N） | `GET /sessions/:id/turns?after=N` | `sumeru session turns <id> --after N` | ✅ atest: `turns/turns-pagination.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 6.1 | 列出 providers | `GET /providers` | `sumeru provider list` | ✅ atest: `provider/crud-lifecycle.yaml` |
| 6.2 | 获取 provider 详情 | `GET /providers/:name` | `sumeru provider get <name>` | ✅ atest: `provider/crud-lifecycle.yaml` |
| 6.3 | 创建 provider | `PUT /providers/:name` | `sumeru provider add <name> --api-type --base-url` | ✅ atest: `provider/crud-lifecycle.yaml` |
| 6.4 | 更新 provider | `PUT /providers/:name` | `sumeru provider update <name> --api-type/--base-url/--api-key` | ✅ atest: `provider/crud-lifecycle.yaml` |
| 6.5 | 删除 provider | `DELETE /providers/:name` | `sumeru provider remove <name>` | ✅ atest: `provider/crud-lifecycle.yaml` |
| 6.6 | 删除被 Model 引用的 provider（409） | `DELETE /providers/:name` → 409 | `sumeru provider remove <name>` → error | ✅ atest: `provider/crud-lifecycle.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 7.1 | 列出 models | `GET /models` 或 `GET /providers/:name/models` | `sumeru model list [--provider <name>]` | ✅ atest: `model/crud-lifecycle.yaml` |
| 7.2 | 获取 model 详情 | `GET /providers/:name/models/:modelName` | `sumeru model get <provider:name>` | ✅ atest: `model/crud-lifecycle.yaml` |
| 7.3 | 创建 model | `PUT /providers/:name/models/:modelName` | `sumeru model add <provider:name> --model <api-model>` | ✅ atest: `model/crud-lifecycle.yaml` |
| 7.4 | 更新 model | `PUT /providers/:name/models/:modelName` | `sumeru model update <provider:name> --model/--context-window` | ✅ atest: `model/crud-lifecycle.yaml` |
| 7.5 | 删除 model | `DELETE /providers/:name/models/:modelName` | `sumeru model remove <provider:name>` | ✅ atest: `model/crud-lifecycle.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 8.1 | 列出 personas | `GET /personas` | `sumeru persona list` | ✅ atest: `persona/crud-lifecycle.yaml` |
| 8.2 | 获取 persona 详情 | `GET /personas/:name` | `sumeru persona get <name>` | ✅ atest: `persona/crud-lifecycle.yaml` |
| 8.3 | 创建 persona | `PUT /personas/:name` | `sumeru persona add <name> --instructions` | ✅ atest: `persona/crud-lifecycle.yaml` |
| 8.4 | 删除 persona | `DELETE /personas/:name` | `sumeru persona remove <name>` | ✅ atest: `persona/crud-lifecycle.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 9.1 | 列出 prototypes | `GET /prototypes` | `sumeru prototype list` | ✅ atest: `prototype/crud-lifecycle.yaml` |
| 9.2 | 删除 prototype | `DELETE /prototypes/:name` | `sumeru prototype remove <name>` | ✅ atest: `prototype/crud-lifecycle.yaml` |
...
| # | 场景 | API | CLI | Spec |
|---|------|-----|-----|------|
| 10.1 | 列出 adapters | `GET /adapters` | `sumeru adapter list` | ✅ atest: `adapter/adapter-list.yaml` |
| 10.2 | 获取 adapter 详情 | `GET /adapters/:name` | `sumeru adapter get <name>` | ✅ atest: `adapter/adapter-list.yaml` |
...
| 11.2 | 容器内执行 shell | `POST /sessions/:id/commands` `{"type":"exec",...}` | `sumeru session exec <id> -- <command...>` | ✅ atest: `commands/session-commands.yaml` |
| 11.3 | 切换 model | `POST /sessions/:id/commands` `{"type":"model",...}` | `sumeru session model <id> <model-id>` | ✅ atest: `commands/session-commands.yaml` |
| 11.4 | 清上下文 | `POST /sessions/:id/commands` `{"type":"reset",...}` | `sumeru session reset <id> [--persona]` | ✅ atest: `commands/session-commands.yaml` |
| 11.5 | snapshot（docker commit） | `POST /sessions/:id/commands` `{"type":"snapshot",...}` | `sumeru session snapshot <id> <name>` | ✅ atest: `commands/session-commands.yaml` |
| 11.6 | snapshot 输出可读性 | — | `sumeru session snapshot` 多行格式 | ✅ atest: `commands/session-commands.yaml` |
...
| 14.4 | Host 重启后 session 上下文恢复 | — (adapter JSONL 持久化) | `server restart` → `session send` 上下文不丢 | ✅ atest: `session/resume-after-restart/resume-after-restart.yaml` |
