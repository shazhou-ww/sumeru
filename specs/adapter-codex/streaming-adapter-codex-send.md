...
- The contributor rewrites `packages/adapter-codex/src/adapter.ts` and `packages/adapter-codex/src/stream-parser.ts`:
  ...
  - On a **timeout** (`exitInfo.timedOut === true`, at `packages/adapter-codex/src/adapter.ts:322-327`), yields `{ type: "suspend", reason: "timeout", nativeId, elapsedMs: exitInfo.durationMs }` and `return`s — NOT an `error` event. `nativeId` is `ref.nativeId` (held since send entry); `elapsedMs` is the spawn's `exitInfo.durationMs`.
...
- On a **send timeout** (`exitInfo.timedOut === true`):
  - Yields exactly one `{ type: "suspend", reason: "timeout", nativeId, elapsedMs }` as the **last** event, then `return`s — NO `error`, NO `done`. `nativeId` is `ref.nativeId`; `elapsedMs` is `exitInfo.durationMs` (a number). The codex process is still SIGTERM→SIGKILLed by the spawn timer. Already-yielded turns remain in cache.
- On other failures:
  - Yields `{ type: "error", error: Error }` and terminates. Already-yielded turns remain in cache.
- The timeout unit test in `packages/adapter-codex/tests/send.test.ts` (formerly "yields error event on send timeout") asserts `event.type === "suspend"` with `reason === "timeout"`, a non-empty `nativeId`, and numeric `elapsedMs`.
