...
Persona   ── 它是谁？（system prompt + skills）
   │
Prototype ── 组装成可运行的 agent 模板（persona + model + adapter）
   │
...
| **Prototype** | YAML 文件 | 组装层：`persona` + `model` + `adapter` + 资源 defaults |
...
自动 Docker build，生成 `sumeru/<adapter>:dev` 标签。支持的 agent type：`sarsapa`、`hermes`、`claude-code`、`codex`。镜像标签写在 `prototypes/<name>/compose.yaml` 中。

### 4. 注册 Prototype

```bash
sumeru prototype add sarsapa --model deepseek-v3 --adapter sarsapa --persona default
```
...
请求 body：`{ "name": "...", "persona": "...", "model": "...", "adapter": "..." }`

创建时校验 `persona`、`model` 引用均存在。Docker 镜像由 `prototypes/<name>/compose.yaml` 指定。Prototype 也可通过 `data/prototypes/*.yaml` 文件加载。
...
| `image_not_found` | 400 | Docker 镜像不存在（需先 `sumeru image build`） |
