/**
 * Default `SpawnFn` for the Claude Code adapter — wraps
 ...
 *
 * The consumer iterates `lines` (via `for await`) to receive one complete
 * NDJSON line per iteration BEFORE the child exits. After consuming all
 * lines, call `waitForExit()` to get the exit metadata (code, signal,
 * stderr, timedOut, durationMs).
 *
 * Timeout escalation is identical to `defaultSpawn`: SIGTERM at timeout,
 * SIGKILL after 5 s grace.
 */
