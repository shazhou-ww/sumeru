...
describe("parseStreamJson", () => {
	...
	describe("edit-tool fixture", () => {
		it("maps editToolCall started+completed to WireToolCall with output", () => {
			...
			const turnWithTools = result?.turns.find(
				(t) =>
					t.role === "assistant" &&
					t.toolCalls !== null &&
					(t.toolCalls as unknown[]).length > 0,
			);
			expect(turnWithTools).toBeDefined();
			expect(turnWithTools?.toolCalls?.[0]?.id).toBe("call_edit_001");
		...
		it("emits a progressive ToolTurnValue for the completed tool_call", () => {
			const text = loadFixture("ca-stream.edit-tool.ndjson");
			const result = parseStreamJson(text);
			expect(result).not.toBeNull();
			// user + assistant + tool + assistant = 4
			expect(result?.turns.length).toBe(4);
			const toolTurn = result?.turns.find((t) => t.role === "tool");
			expect(toolTurn).toBeDefined();
			expect(toolTurn?.role).toBe("tool");
			expect((toolTurn as { callId?: string })?.callId).toBe("call_edit_001");
			expect((toolTurn as { name?: string })?.name).toBe("editToolCall");
			expect((toolTurn as { result?: string })?.result).toBe(
				"File created successfully",
			);
		});
	...
	describe("shell-tool fixture", () => {
		it("maps shellToolCall started+completed to WireToolCall with stdout and exitCode", () => {
			...
			expect(turnWithTools).toBeDefined();
			expect(turnWithTools?.toolCalls?.[0]?.id).toBe("call_shell_001");
		...
		it("emits a progressive ToolTurnValue carrying exitCode-aware result", () => {
			const text = loadFixture("ca-stream.shell-tool.ndjson");
			const result = parseStreamJson(text);
			expect(result).not.toBeNull();
			const toolTurn = result?.turns.find((t) => t.role === "tool");
			expect(toolTurn).toBeDefined();
			expect((toolTurn as { callId?: string })?.callId).toBe("call_shell_001");
			expect((toolTurn as { result?: string })?.result).toContain("hello.ts");
		});
	});
	...
	describe("tool-call pairing edge cases", () => {
		it("unmatched started keeps output null", () => {
			...
			// No progressive tool turn since completed never arrived.
			expect(result?.turns.find((t) => t.role === "tool")).toBeUndefined();
		});
		...
		it("unmatched completed is silently dropped", () => {
			...
			// No progressive tool turn for an unmatched completed.
			expect(result?.turns.find((t) => t.role === "tool")).toBeUndefined();
		});
		...
		it("multiple tool_call events between two assistant lines all attach to the most recent", () => {
			...
			// Two progressive tool turns, one per completed.
			const toolTurns = result?.turns.filter((t) => t.role === "tool") ?? [];
			expect(toolTurns.length).toBe(2);
		});
	});
...
describe("parseStreamJsonIncremental", () => {
	async function collect(lines: AsyncIterable<string>): Promise<{
		meta: { sessionId: string; model: string } | null;
		turns: unknown[];
		resultLine: Record<string, unknown> | null;
	}> {
		const meta = null as { sessionId: string; model: string } | null;
		let m = meta;
		const turns: unknown[] = [];
		let resultLine: Record<string, unknown> | null = null;
		for await (const event of parseStreamJsonIncremental(lines)) {
			if (event.type === "meta") {
				m = { sessionId: event.sessionId, model: event.model };
		...
		return { meta: m, turns, resultLine };
	}

	async function* toLines(text: string): AsyncGenerator<string> {
		for (const line of text.split("\n")) {
			yield line;
		}
	}

	it("yields meta first, then turns, then result for the simple fixture", async () => {
		const text = loadFixture("ca-stream.simple.ndjson");
		const { meta, turns, resultLine } = await collect(toLines(text));
		...
		expect((resultLine as { subtype: string }).subtype).toBe("success");
	});

	it("incremental turn count matches batch parse for the edit-tool fixture", async () => {
		const text = loadFixture("ca-stream.edit-tool.ndjson");
		const batch = parseStreamJson(text);
		const { turns } = await collect(toLines(text));
		expect(batch).not.toBeNull();
		expect(turns.length).toBe(batch?.turns.length ?? -1);
		// The progressive tool turn is emitted between the two assistant turns.
		expect((turns[2] as { role: string }).role).toBe("tool");
	});

	it("meta is emitted exactly once even if system line repeats session id", async () => {
		const text = [
			JSON.stringify({
				type: "system",
				subtype: "init",
		...
		const { meta } = await collect(toLines(text));
		expect(meta?.sessionId).toBe("once-001");
	});

	it("tolerates malformed lines incrementally", async () => {
		const text = loadFixture("ca-stream.malformed.ndjson");
		const { meta, turns, resultLine } = await collect(toLines(text));
		expect(meta?.sessionId).toBe("malformed-session-001");
		expect(turns.length).toBe(2);
		expect(resultLine).not.toBeNull();
	});
});

describe("doneValueFromResultLine", () => {
	it("returns null summary + null tokenUsage for a null result line", () => {
		const out = doneValueFromResultLine(null);
		expect(out.summary).toBeNull();
		expect(out.tokenUsage).toBeNull();
	});

	it("maps cursor-agent usage fields to {input, output, cached}", () => {
		const out = doneValueFromResultLine({
			type: "result",
			subtype: "success",
			result: "Hello!",
			duration_ms: 100,
		...
		expect(out.tokenUsage).toEqual({ input: 150, output: 25, cached: 7 });
	});

	it("returns null tokenUsage when all usage fields are zero", () => {
		const out = doneValueFromResultLine({
			type: "result",
			subtype: "success",
			result: "x",
	...
	it("falls back to null summary when result is not a string", () => {
		const out = doneValueFromResultLine({
			type: "result",
			subtype: "success",
		...
		expect(out.summary).toBeNull();
		expect(out.tokenUsage).toEqual({ input: 5, output: 0, cached: 0 });
	});
});
